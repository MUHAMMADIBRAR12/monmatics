                            ...
                            <div class="col-lg-6 mb-3">
                                ...
                                <select name="leave_for" id="leave_for" class="form-control">
                                    ...
                                    @foreach($designations as $designation)
                                        <option value="{{ $designation->id }}" @if($designation->id == $leave->designation_id) selected @endif>{{ $designation->designation }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            @foreach($leaveTypes as $type)
                                <div class="row col-lg-12 leave-type">
                                    <div class="col-4 ">
                                        ...
                                        <input class="form-control" name="leave_type[]" type="text" placeholder="Leave Type" value="{{ $type->leave_type }}" required>
                                    </div>
                                    <div class="col-4 ">
                                        <label class="form-check-label" for="leave_count[]">Leaves Count</label>
                                        <input class="form-control" name="leave_count[]" type="text" placeholder="Leave Count" value="{{ $type->leave_count }}" required>
                                    </div>
                                    <div class="col-4 mb-1">
                                        ...
                                        <select class="form-control" name="leave_category[]" required>
                                            <option> Select </option>
                                            <option value="Accural" {{ $type->leave_category == 'Accural' ? 'selected' : '' }}>Accural</option>
                                            <option value="Earned" {{ $type->leave_category == 'Earned' ? 'selected' : '' }}>Earned</option>
                                            <option value="Carry_Forward" {{ $type->leave_category == 'Carry_Forward' ? 'selected' : '' }}>Carry Forward</option>
                                        </select>
                                    </div>
                                </div>
                            @endforeach
