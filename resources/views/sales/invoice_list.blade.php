                        ...
                        <div class="col-md-12">
                            <table>
                                <tr>
                                    <td>Customer:</td>
                                    <td>
                                        <input type="text" id="customer" onkeyup="autoFill(this.id,CustomerURL,token)"
                                            class="form-control" style="height:30px;width:230px">
                    ...
                    <table class="table table-bordered table-striped table-hover" id="invoices" style="width:100%">
                        ...
                        <tbody id="invoice_body">
                            ...
                            <tr>
                                ...
                                <td class="column_size text-center">{{  date(appLib::showDateFormat(), strtotime($invoice->date))}}</td>
                                <td class="column_size text-center">{{  date(appLib::showDateFormat(), strtotime($invoice->due_date))}}</td>
            ...
            buttons: [
                ...
                customize: function (csv) {
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        // Remove the action column (index 0) from the row
                        columns.splice(0, 1);
                        // Join the modified columns back into a row
                        rows[i] = columns.join(',');
                    }
                    // Join all rows back into the CSV string
                    csv = rows.join('\n');
                    return csv;
                }
                },
                {
                    ...
                    customize: function(doc) {
                        doc.content[1].table.body.forEach(function(row) {
                            row.splice(0, 1); // Remove the first column (action column) from each row
                        });
                    }
                },
                {
                    ...
                    customize: function(xlsx) {
                        ...
                        $('row c[r^="A"]', sheet).remove(); // Remove the action column from the Excel sheet
                    }
                },
                {
                    extend: 'print',
...
<script>
    ...
    $.post(url, {
        ...
        data.map(function(val, i) {
            ...
            var printBtn = '<button class="btn btn-danger btn-sm p-0 m-0" onclick="window.open(\'' +
                '{{ url('Sales/Invoice/Create/pdf/') }}' + '/' + val.id + '\');"><i class="zmdi zmdi-receipt px-2 py-1"></i></button>';
            var actionColumn = '<button class="btn btn-success btn-sm p-0 m-0" onclick="goTo(\'' +
                finalUrl + '\')"><i class="zmdi zmdi-edit px-2 py-1"></i></button>' +
                '<button class="btn btn-primary btn-sm p-0 m-0" onclick="goTo(\'' +
                '{{ url('Sales/Invoice/View/c/') }}' + '/' + val.id + '\')"><i class="zmdi zmdi-eye px-2 py-1"></i></button>' +
                printBtn;

            var row = '<tr class="rowData">' +
                '<td class="action">' + actionColumn + '</td>' +
                '<td>' + val.inv_num + '</td>' +
                '<td>' + val.name + '</td>' +
                '<td>' + val.date1 + '</td>' +
                '<td>' + val.date2 + '</td>' +
