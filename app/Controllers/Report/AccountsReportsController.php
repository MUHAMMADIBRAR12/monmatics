...
class AccountsReportsController extends Controller
{
    ...
    public function customerOutstandingDetail(Request $request)
    {
        ...
        $customer = DB::table('sal_invoices')
            ...
            ->select('crm_customers.name', 'sal_invoices.id' ,'sal_invoices.date', 'sal_invoices.due_date', DB::raw("concat(sal_invoices.month,'-',LPAD(sal_invoices.number,4,0)) as inv_num"), 'sal_invoices.total_inv_amount', 'sal_invoices.amount_received')
    ...
    public function printVoucher($trmId)
    {
        ...
        if ($transmains) {
            ...
            $pdf->SetXY($X, $Y);
            $pdf->setT1(10);
            $pdf->Cell($VW, $LH, appLib::setDateFormat($transmains->date), $B, 0, 'L', 0, '', 0);
            $Y = $pdf->GetY();
            $X = $pdf->GetX();
            $pdf->Ln($Ln + 4);
