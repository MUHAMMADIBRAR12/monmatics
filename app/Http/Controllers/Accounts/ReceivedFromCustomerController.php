...
class ReceivedFromCustomerController extends Controller
{
    ...
    public function customerInvoices(Request $request)
    {
        ...
        $invoices = DB::table('sal_invoices')
            ->select('sal_invoices.id', DB::raw("concat(sal_invoices.month,'-',LPAD(sal_invoices.number,4,0)) as inv_num"), 'sal_invoices.due_date', DB::raw('sal_invoices.total_inv_amount - sal_invoices.amount_received as balance'),'sal_invoices.subject')
            ->where('sal_invoices.cst_coa_id', $request->coa_id)
            ->where('sal_invoices.cur_id', $request->currency)
    ...
    public function receivedAmountSave(Request $request)
    {
        ...
        try{
            ...
            // throw new Exception('Manually Checking By Zeeshan');
            // dd('stop');
            DB::commit();
