...
class DailyRecoveryController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        // dd('outer');
            ...
            // throw new Exception('manually error from Zeeshan');
            DB::commit();

            return redirect('Accounts/DailyRecorvery/List')->with('message','Recovery Added Successfully');
        } catch (Exception $e) {
            // dd('rollback');
            DB::rollBack();
    ...
    public function report(Request $request)
    {
        ...
        $daily_recovery_list = DB::table('fs_daily_recovery')
            ...
            ->join('fs_coas as fs1', 'fs1.id', '=', 'fs_daily_recovery.received_coa_id')
            ->join('fs_coas as fs2', 'fs2.id', '=', 'fs_daily_recovery.cst_coa_id')
            ->select('fs_daily_recovery.*', DB::raw("DATE_FORMAT(fs_daily_recovery.created_at, '$mysqlDateFormat') as timefor"), DB::raw("concat(fs_daily_recovery.month,'-',LPAD(fs_daily_recovery.number,4,0)) as recovery_num"), 'fs1.name as received_in', 'fs2.name as received_from', 'fs_transmains.post_status')
