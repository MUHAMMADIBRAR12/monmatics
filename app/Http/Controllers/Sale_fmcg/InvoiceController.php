...
class InvoiceController extends Controller
{
    ...
    public function index($type=null)
    {
        $Invoices = DB::table('sal_invoices')
                        ...
                        ->orderByRaw('month, number ASC')->get();
        
        return view('sale_fmcg.invoice_list', compact('Invoices','type'));
    }
    ...
    public function form($type=null,$id=null)
    {
        ...
        if($id)
            return view('sale_fmcg.invoice', compact( 'SalesAccounts', 'paymentTerms', 'taxList','discountList', 'projects','warehouses','Invoice','InvoiceDetail', 'attachmentRecord','recurings','invoice_tax','type'));
        else
            return view('sale_fmcg.invoice', compact('SalesAccounts', 'paymentTerms', 'taxList','discountList', 'projects','warehouses','recurings','type'));
    }
    ...
    public function view($type=null,$id=null)
    {
        ...
        */
        ...
        return view('sale_fmcg.invoice_views', compact( 'Invoice','InvoiceDetail', 'attachmentRecord','invoice_tax','type'));
    ...
    public function printinvoice($type=null,$id=null)
    {
       ...
       if($id)
       {
           $Invoicepdf = DB::table('sal_invoices')
                           ...
                           ->leftJoin('sal_quotation as q', 'q.id','=', 'quot_id')
                           ->select('crm_customers.name as crm_name','sal_invoice_tax.net_payable','sal_invoice_tax.disc_rate','sal_invoice_tax.advance_amount','sal_invoice_tax.disc_amount','sal_invoice_tax.tax_rate as rate','tax_amount','fs_coas.name as tax_name','f1.name as disc_name', DB::raw("concat(sal_invoices.month,'-',LPAD(sal_invoices.number,4,0)) as inv_num") ,'q.month as qmonth','crm_customers_address.phone','q.number as qnumber', 'sal_invoices.*','crm_customers_address.address','crm_customers_address.location')
                           ->where('sal_invoices.id','=',$id)
      ...
      // dd($Invoice);
       ...
       $pdf->SetXY($X, $Y);
       $pdf->setTitleFont();
       $pdf->Cell($pdf->getPageWidth()-($X*2), 10, 'INVOICE', $B, 0, 'C', 0, 'B', 0);
       $Y=$pdf->GetY()+18;
       $pdf->SetXY($X,$Y);        
       $pdf->setH3();
...
//style="border: 1px solid black"
    ...
    <table class="table" style=" font-family: Arial, Helvetica, sans-serif; width: 100%; "> 
    <tr class="bg" style=" border: 1px solid #ddd; padding: 8px;">
       ...
       <th style="width:70; $titleFont ">Amount</th>
    </tr>
    $PDFLineItem
</table>
EOD;
   ...
   $X=118; $Ln=5; $B=1; $LH=6; $LW=50; $VW=30;
   ...
   $pdf->SetXY($X+$LW,$Y);
   $pdf->setT1();
   $pdf->MultiCell($VW, $Ln,Str::currency($total), 'LRB', 'R', 0, 0, '', '', true, 0, false, true, 0);
   $pdf->Ln();

   if($Invoicepdf->tax_amount>0)
   {
       $Y=$pdf->GetY();
       $pdf->setH3();
       $pdf->SetXY($X,$Y);
       $pdf->MultiCell($LW, $Ln,$Invoicepdf->tax_name.'@'.Str::currency($Invoicepdf->rate), 'LB', 'R', 0, 0, '', '', true,0, false, true, 0);
       $pdf->SetXY($X+$LW,$Y);
       $pdf->setT1();
       $pdf->MultiCell($VW, $Ln,Str::currency($Invoicepdf->tax_amount), 'LRB', 'R', 0, 0, '', '', true, 0, false, true, 0);
       $pdf->Ln();
   }
   ...
   $pdf->SetXY($X,$Y);
   $pdf->setT1();
   $pdf->MultiCell(90, $Ln,$Invoicepdf->note, 0, 'L', 0, 0, '', '', true, 0, false, true, 0);   
