...
class CallsController extends Controller
{
    ...
    public function getCallsList(Request $request)
    {
        ...
        foreach ($calls as $call) {
            ...
            if ($call->related_to_type == 'contacts') {
                $contact = DB::table('crm_contacts')->select('first_name', 'id')->where('id', $call->related_id)->first();
                $related_to = ($contact) ? array('first_name' => $contact->first_name, 'id' => $contact->id) : '';
            } elseif ($call->related_to_type !== 'all') {
                $table = appLib::$related_table[$call->related_to_type];
                $related_record = ($table) ? DB::table($table)->select('name', $field)->where($field, $call->related_id)->first() : null;
            ...
            $call_arr1 = array(
                "id" => $call->id,
                "subject" => $call->subject,
                "start_date" => date(appLib::showDateFormat(), strtotime($call->start_date)),
