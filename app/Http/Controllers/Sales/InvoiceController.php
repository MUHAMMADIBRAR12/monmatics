...

{
...

    {
        ...
        if ($id) {
            ...
            $Invoicepdf = DB::table('sal_invoices')
                ...
                ->select(
                    'crm_customers.name as crm_name',
                    'sal_invoice_tax.net_payable',

                    'sal_invoice_tax.disc_rate',
                    'sal_invoice_tax.advance_amount',
                    'sal_invoice_tax.disc_amount',
                    'sal_invoice_tax.tax_rate as rate',
                    'tax_amount',
                    'fs_coas.name as tax_name',
                    'f1.name as disc_name',
                    DB::raw("concat(sal_invoices.month,'-',LPAD(sal_invoices.number,4,0)) as inv_num"),
                    'q.month as qmonth',
                    'crm_customers_address.phone',
                    'q.number as qnumber',
                    'sal_invoices.*',
                    'crm_customers_address.address',
                    'crm_customers_address.location',
                    'sys_currencies.name as currency_name',
                    'sys_currencies.symbol'
                ) // Added currency_name and symbol columns
    ...
    public function report(Request $request)
    {
        ...
        $Invoices = DB::table('sal_invoices')
            ...
            ->select(
                'sal_invoices.*',
                DB::raw("DATE_FORMAT(sal_invoices.date, '$mysqlDateFormat') as date1"),
                DB::raw("DATE_FORMAT(sal_invoices.due_date, '$mysqlDateFormat') as date2"),
                'crm_customers.name',
                DB::raw("concat(sal_invoices.month,'-',LPAD(sal_invoices.number,4,0)) as inv_num")
            )
