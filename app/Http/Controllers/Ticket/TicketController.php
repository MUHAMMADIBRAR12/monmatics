...
class TicketController extends Controller
{
    ...
    public function updateTicket(request $request)
    {
        ...
        $data = array(
            ...
            'created_at' => appLib::setDateFormat(Carbon::now()),
    ...
    public function BulkTicket(Request $request)
    {
        ...
        if (!empty($ids)) {
            ...
            if ($request->status == 'Delete') {
                // if ($request->assign_id_value) {
                //     DB::table('tkt_tickets')->whereIn('id', $ids)->update(['assign_to'=> $request->assign_id_value]);
                // }
    ...
    public function TicketSearchReports(Request $request)
    {
        ...
        if ($request->anyFilled(['ticket_id', 'email', 'priority', 'status', 'category', 'department', 'from_date', 'to_date', 'assign_ID', 'related_ID', 'is_billable'])) {
            ...
            $subquery = DB::table('tkt_tickets_history')
                ->select('tkt_id', DB::raw('SEC_TO_TIME(SUM(TIME_TO_SEC(total_time))) as total_history_time'))
                ->groupBy('tkt_id');

            $tickets = $ticketsQuery
                ->leftJoin('users', 'users.id', 'tkt_tickets.assign_to')
                ->leftJoinSub($subquery, 'history', function ($join) {
                    $join->on('tkt_tickets.id', '=', 'history.tkt_id');
                })
                ->select(
                    'tkt_tickets.*',
                    'users.firstName as assigned_user_first_name',
                    'users.lastName as assigned_user_last_name',
                    'history.total_history_time'
                )
                ->where('tkt_tickets.company_id', session('companyId'))
                ->orderBy('tkt_tickets.number', 'asc')
                ->get();
        } else {
