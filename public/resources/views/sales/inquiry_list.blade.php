        ...
        <div class="card">
            <div class="body">
                <div class="table-responsive">
                    <button class="btn btn-primary" style="align:right" onclick="window.location.href = '{{ url('Sales/Inquiry/Create') }}';" >New Inquiry</button>
                    <table class="table table-bordered table-striped table-hover" id="inquires">
                        <thead>
                            <tr>
                            ...
                            @foreach($Inquiries as $inquiry)
                                <tr >
                                    <td class="action">
                                        <button class="btn btn-success btn-sm p-0 m-0" onclick="window.location.href = '{{ url('Sales/Inquiry/Create/'.$inquiry->id) }}';"><i class="zmdi zmdi-edit px-2 py-1"></i></button>
                                    </td>
                                    <td class="column_size">{{ $inquiry->month . '-' .$inquiry->number }}</td>
                                    <td class="column_size">{{ $inquiry->name }}</td>
                                    <td class="column_size">{{  date(appLib::showDateFormat(), strtotime($inquiry->date))}}</td>
                                    <td class="column_size">{{  $inquiry->status }}</td>
